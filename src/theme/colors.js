export const lightTheme = {
  mode: 'light',
  primaryLight: '#E8EBFF',
  primary: '#536DFE',
  primaryDark: '#3756FF',
  secondary: '#ff6f00',
  accent: '#A55617',
  black: '#000',
  white: '#fff',
  lightGrey: '#C3C7D2',
  noColor: '#E1E5EC',
  lightestGrey: '#f0f0f0',
  wearGoalProgressTrack: '#E2E5EC',
  bg: '#F1F0F5',
  error: '#bd413d',
  green: '#42c229',
  gray: 'gray',
  red: '#C82828',
  placeholder: '#9A9A9A',

  Tops: '#3399FF',
  Bottoms: '#4CCB6B',
  One: '#8A5ECD',
  Outerwear: '#E2616B',
  Intimates: '#ED6B9F',
  Footwear: '#FFA348',
  Accessories: '#4ED8A8',
  Other: '#8A9399',

  itemColorOptions: [
    { name: 'Black', hex: '#202124' },
    { name: 'White', hex: '#FFFFFF' },
    { name: 'Grey', hex: '#AEB7C2' },
    { name: 'Brown', hex: '#A56A43' },
    { name: 'Beige', hex: '#E5D0AD' },
    { name: 'Red', hex: '#FF4D4F' },
    { name: 'Pink', hex: '#EC4899' },
    { name: 'Orange', hex: '#FF9F1C' },
    { name: 'Yellow', hex: '#FFD43B' },
    { name: 'Green', hex: '#22C55E' },
    { name: 'Blue', hex: '#3B82F6' },
    { name: 'Purple', hex: '#A855F7' },
  ],

  wearLevels: {
    unworn: {
      bg: '#ECEFF1',
      text: '#607D8B',
    },
    new_arrival: {
      bg: '#E8F5E9',
      text: '#2E7D32',
    },
    lightly_worn: {
      bg: '#E3F2FD',
      text: '#1565C0',
    },
    in_rotation: {
      bg: '#E8EBFF',
      text: '#3756FF',
    },
    well_worn: {
      bg: '#FFF3E0',
      text: '#C45A00',
    },
    wardrobe_mvp: {
      bg: '#FFF8E1',
      text: '#9A6A00',
    },
    legacy_item: {
      bg: '#ECEFF1',
      text: '#455A64',
    },
  },
  wearGoalProgressRange: [
    {
      percent: 1,
      bg: '#FFE0D1',
      text: '#E94B16',
    },
    {
      percent: 35,
      bg: '#FFEBA6',
      text: '#D98F00',
    },
    {
      percent: 65,
      bg: '#DDF9A6',
      text: '#65B600',
    },
    {
      percent: 100,
      bg: '#B8F5CD',
      text: '#009E4C',
    },
  ],

  textSectionTitleColor: '#b6c1cd',
  dayTextColor: '#2d4150',
  textDisabledColor: '#a1a1a1',

  confirmationModal: '#4CAF50',
  progressTintColor: '#51fa05',
  tintColorSecondary: '#f03702',
};

export const darkTheme = {
  mode: 'dark',
  primaryLight: '#2c2f45',
  primary: '#536DFE',
  primaryDark: '#4763fd',
  secondary: '#FF8F33',
  accent: '#FFD54F',
  black: '#FFFFFF',
  white: '#121212',
  lightGrey: '#2E2E36',
  noColor: '#3A3A44',
  lightestGrey: '#2C2C2E',
  wearGoalProgressTrack: '#242733',
  bg: '#1A1A1A',
  error: '#bd413d',
  green: '#66DD55',
  gray: '#B0BEC5',
  red: '#F0625D',
  placeholder: '#B3B3B3',

  Tops: '#66B2FF',
  Bottoms: '#6BE88B',
  One: '#B39DDB',
  Outerwear: '#F48FB1',
  Intimates: '#F8BBD0',
  Footwear: '#FFCC80',
  Accessories: '#80EEC9',
  Other: '#B0BEC5',

  itemColorOptions: [
    { name: 'Black', hex: '#2A2D33' },
    { name: 'White', hex: '#F6F6F6' },
    { name: 'Grey', hex: '#A7ADB7' },
    { name: 'Brown', hex: '#A97852' },
    { name: 'Beige', hex: '#CDB794' },
    { name: 'Red', hex: '#F87171' },
    { name: 'Pink', hex: '#E879B4' },
    { name: 'Orange', hex: '#F59E0B' },
    { name: 'Yellow', hex: '#EACB52' },
    { name: 'Green', hex: '#6BCF83' },
    { name: 'Blue', hex: '#6E9AE0' },
    { name: 'Purple', hex: '#B38AE8' },
  ],

  wearLevels: {
    unworn: {
      bg: '#263238',
      text: '#CFD8DC',
    },
    new_arrival: {
      bg: '#17351F',
      text: '#8BE29A',
    },
    lightly_worn: {
      bg: '#12304A',
      text: '#8DCBFF',
    },
    in_rotation: {
      bg: '#252A55',
      text: '#AEB8FF',
    },
    well_worn: {
      bg: '#3D2A12',
      text: '#FFC06A',
    },
    wardrobe_mvp: {
      bg: '#3D320B',
      text: '#FFE27A',
    },
    legacy_item: {
      bg: '#263238',
      text: '#CFD8DC',
    },
  },
  wearGoalProgressRange: [
    {
      percent: 1,
      bg: '#4A241B',
      text: '#E16F48',
    },
    {
      percent: 35,
      bg: '#4A3211',
      text: '#E8A32B',
    },
    {
      percent: 65,
      bg: '#2D4315',
      text: '#95D849',
    },
    {
      percent: 100,
      bg: '#123F22',
      text: '#50CF78',
    },
  ],

  textSectionTitleColor: '#b6c1cd',
  dayTextColor: '#a1a1a1',
  textDisabledColor: '#2d4150',

  confirmationModal: '#4CAF50',
  progressTintColor: '#70FF4F',
  tintColorSecondary: '#FF9E80',
};
